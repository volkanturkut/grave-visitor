...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        public void LockInput(bool lockInput)
        {
            ...
            if (_inputLocked)
            {
                // Reset movement values so character stops immediately
                _speed = 0f;
                _animationBlend = 0f;
                if (_hasAnimator)
                {
        ...
        {
            if (_hasAnimator) _animator.SetTrigger(triggerName);
        }
        ...
        {
            if (_inputLocked) return; // Don't rotate camera if locked (optional)

            if (_input.cameraDrag && _input.move == Vector2.zero) return;
        ...
        private void JumpAndGravity()
        {
            if (Grounded)
            {
                ...
                // Disable Jump if Input is Locked
                if (_input.jump && _jumpTimeoutDelta <= 0.0f && !_inputLocked)
                {
