...
public class GraveDirtController : MonoBehaviour, IInteractable
{
    ...
    private void InitializeMaterial()
    {
        ...
        {
            DebugLogger.LogError($"[GraveDirtController] Could not find dirt material at index {dirtMaterialIndex} on {name}");
            enabled = false;
        }
    }
    ...
    /// <summary>
    /// Called when player interacts with the grave to clean it
    /// </summary>
    public void Interact(Transform interactorTransform)
    {
        if (_isBeingCleaned) return;

        // Don't clean if it's already clean
        if (_currentDirtAmount <= 0.05f)
        {
            DebugLogger.Log("This grave is already clean.");
            return;
        }
        ...
        if (inventory != null)
        {
            ...
            {
                DebugLogger.Log($"You need a Brush! (Held: {(equippedItem != null ? equippedItem.itemName : "Nothing")})");
            }
        }
        else
        {
            DebugLogger.LogError("CRITICAL: No InventoryManager found in the scene!");
        }
    }
